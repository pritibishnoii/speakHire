...
const InterviewCard = async ({
  ...
  const feedback =
    userId && interviewId
      ...
      console.log("feedbacck from interviw card component ---",feedback)
  const cleanType = typeof type === 'string' ? type.trim() : "";
  const normalizedType = cleanType ? (/mix/gi.test(cleanType) ? "Mixed" : cleanType) : "Mixed";
  ...
  let dateValue = feedback?.createdAt || createdAt || Date.now();
  ...
  const formattedDate = dayjs(dateValue).format("MMM D, YYYY");
